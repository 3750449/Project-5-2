...
//            std::cout << '0';
        ...
        //1 = USED, 0 = AVAILABLE - my attempt
        const bool occupied = (hashTableMask[i] == USED) || (hashTableMask[i] == DELETED);
        std::cout << (occupied ? '1' : '0');

        if (++countOnLine == width) {
            std::cout << "\n";
            countOnLine = 0;
